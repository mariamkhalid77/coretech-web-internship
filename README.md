
# CoreTech Web Development Internship

## Tasks

- Task 1: ACCOUNT SETUP-"This task is for project setup and GitHub repository initialization"
- Task 2: HTML PORTFOLIO PAGE-"Creates the HTML structure of the portfolio page"
- Task 3: CSS PORTFOLIO PAGE-"Handles styling and design of the website by usinf CSS"
- Task 4: GIT COMMANDS-"Contains git commands for version control and repository management"
- Task 5: JAVASCRIPT FORM VALIDATION-"Handles form validation using JavaScript"
- Task 6: TO-DO LIST APP-"Builds a To-Do List app with add, delete, and localstorage features"
- Task 7: FETCH USER DATA-"This task fetches and displays data from an API"
- Task 8: PRODUCT LISTING WEBSITE-"Product listing page with Search, Filter , Sort , and responsive design"